...
const Principal = {
    ...
    async inicializar() {
        ...
        Divisas.actualizarReferenciaRapida();
        
        // 7. Ponemos a valer las noticias y análisis (Módulo Modularizado)
        if (window.Noticias) {
            Noticias.init();
        }
    ...
    configurarBotonesInterfaz() {
        ...
        document.querySelectorAll('.quick-amt-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                Divisas.referenciaRapidaMonto = parseInt(btn.getAttribute('data-amt'));
                Divisas.actualizarReferenciaRapida();
            });
        });
        ...
        if (sliderGasofa) sliderGasofa.addEventListener('input', () => Gasolina.refrescarSurtidor());
    },
    ...
    navegar(pantalla) {
        ...
        if (pantalla === 'home-screen') {
            ...
            if (window.Gasolina) Gasolina.obtenerEstacionesCercanas();
        }
    },
    ...
    // Funció Maestra para compartir imagen generada (Permanece en Principal por ser acción global)
