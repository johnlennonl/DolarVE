// ==========================================
// ¡Epa mi brother! Bienvenido a DolarVE App Logic v11.0
// Este código está hecho con puro cariño (y unas buenas arepas).
// Porfa, mosca con romperlo :(
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
    console.log('[DolarVE] ¡Activo y funcionando, papá!');

    // --- CONEXIÓN CON SUPABASE (Nuestra Base de Datos Mágica) ---
    // Mosca aquí mi pana, estas son las llaves para guardar las cuentas del pago móvil
    ...
    // --- PWA INSTALL LOGIC (Fase 3 - Instálalo en el celular) ---
    // Aquí es donde cuadramos para que la app se baje como una nativa
    let deferredPrompt;
    ...
    if (!isStandalone) {
        ...
        if (isIOS) {
            ...
            if (installIcon) { installIcon.className = 'ph-duotone ph-apple-logo'; installIcon.style.color = '#fff'; }
    ...
    if (supabase) {
        // Chequeamos si el convive ya había iniciado sesión antes
    ...
    async function fetchData() {
        if (!navigator.onLine) {
            // Se cayó el ABA de Cantv o los datos, sacamos las viejas del baúl
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                ...
                if (currentUsdRate > 0 && currentParaleloRate > 0) {
                    const brecha = ((currentParaleloRate - currentUsdRate) / currentUsdRate) * 100;
                    document.getElementById('brecha-value').innerText = `${brecha.toFixed(2)}%`;
                    const trendIcon = brecha > 0 ? '<i class="ph-duotone ph-chart-line-up" style="font-size: 14px; color: var(--accent-red);"></i>' : '';
        ...
        try {
            ...
            const listContainer = document.getElementById('dynamic-crypto-list');
            if (listContainer) {
                listContainer.innerHTML = '<div style="text-align:center; padding: 20px; color: var(--text-muted);"><i class="ph-duotone ph-cloud-warning" style="margin-bottom:10px; font-size: 20px;"></i><br>Servidor Cripto ocupado.<br>Intenta refrescar en un rato.</div>';
            }
        }
    }

    // ¡Arrancamos los motores! Pedimos todas las tasas de una vez pa' que esto vuele
    fetchData();
    // Crypto Search Logic
    ...
    if (testNotifBtn) {
        testNotifBtn.addEventListener('click', () => {
            ...
            testNotifBtn.innerHTML = '<i class="ph-duotone ph-circle-notch ph-spin"></i> Bloquea tu celular ahora... (20s)';
            ...
            setTimeout(() => {
                ...
                testNotifBtn.innerHTML = '<i class="ph-duotone ph-check"></i> Prueba Enviada';
                testNotifBtn.style.color = "var(--accent-green)";
                testNotifBtn.style.borderColor = "var(--accent-green)";

                setTimeout(() => {
                    testNotifBtn.innerHTML = '<i class="ph-duotone ph-broadcast"></i> Emitir Señal de Prueba (20s)';
                }, 4000);
            }, 20000);
        });
    }
    ...
    function renderAccountsList(accounts) {
        ...
        if (accounts.length === 0) {
            pmAccountsList.innerHTML = '<div style="text-align: center; padding: 40px 20px; color: var(--text-muted); font-size: 13px;"><i class="ph-duotone ph-wallet" style="font-size: 32px; opacity: 0.2; display: block; margin-bottom: 12px;"></i>No tienes cuentas registradas aún.<br>Agrega una para generar recibos de cobro.</div>';
            return;
        }
    ...
    if (savePmBtn) {
        savePmBtn.addEventListener('click', async () => {
            ...
            savePmBtn.innerHTML = '<i class="ph-duotone ph-circle-notch ph-spin"></i> Guardando...';
            ...
            savePmBtn.innerHTML = '<i class="ph-duotone ph-cloud-arrow-up"></i> Guardar en la Nube';
        });
    }
